...
输入文本：
"稻瘟病是水稻最重要的真菌病害之一，由稻瘟病菌引起。
三环唑是防治稻瘟病的常用杀菌剂，每亩用量约为100克。
稻瘟病的典型症状包括叶片出现褐色梭形病斑，严重时可导致减产30%以上。"
...
def _load_entity_aliases(path: str = "") -> Dict[str, str]:
    ...
    PocketGraphRAG/data/entity_aliases.json 仅作为参考示例（水稻领域），
    不会自动应用。
    ...
    if not path:
        # 通用默认：不加载任何领域别名，避免对用户数据施加水稻领域归一化
        return {}
